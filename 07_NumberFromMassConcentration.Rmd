---
title: "07_NumberFromMassConcentration"
...
```{R Create figures folder, warning=F, message=F}
...
Here we provide the code for converting measured SPM microplastics mass concentrations to particle number concentrations as a proof of principle. Details described in RIVM letter report 2026-0069.

For this we take the following steps:

1.  SPM microplastic concentrations (g/kg) is converted to water microplastics concentration (ug/L) using the SPM concentration.

2.  Per polymer we derive the volume of polymer (PP) measured using polymer density estimates.

3.  Using a predefined PSD, e.g. using the power law as derived using FTIR data to calculate the number of particles from the polymer volume.

# Data preparation

## Pyr-GCMS data

Data is prepared in [06_MassDataPrep.md](06_MassDataPrep.md). The suspended matter polymer concentrations are converted to water concentration using suspended matter concentration as measured in the same period as the microplastic sampling. Additionally a correction for microplastic particle recovery for the sediment box and centrifuge approach is applied.

Below table illustrates an example of the dataset from Rijkswaterstaat.

```{r pyrGCMS, echo=FALSE, warning=F, message=F}

pyrGCMS_data <- read_rds(paste0(project_dir, output_dir, "Data/2026-07-01-PP_SPM_Concentrations.rds"))
# only PP concentration data (source? Ivo Freriks)
...
We assume a triangular distribution with 904 kg/m3 as most likely value between 898 kg/m3 and 908 kg/m3 as min and max.

```{r density, echo=FALSE, warning=F, message=F}

PolymerDensity <- tibble(Polymer = "PP",
                  density_kg_m3 = tibble(a=898, b=908, c=904, dist = "triangular" ))

# Add density to the data
pyrGCMS_data <- 
  pyrGCMS_data |> left_join(PolymerDensity)
...
Here a powerlaw is assumed to describe the PP particle size distribution (PSD).

## Calculate using mean particle volume

We can calculate the total volume of microplastic particles (PP) by dividing the measured mass concentration by the density:

$$V_{\text{total}} = \frac{m_{\text{PP}}}{\rho_{\text{PP}}}$$

We know that particle volume is distributed according a powerlaw with a power law slope. We also know the smallest and largest particle in our distribution. We can thus calculate the mean particle volume from the powerlaw distribution:

$$\langle v \rangle = \frac{ \displaystyle \int_{v_{\min}}^{v_{\max}} v \, n(v) \, dv }{ \displaystyle \int_{v_{\min}}^{v_{\max}} n(v) \, dv }$$

$$\langle v \rangle = \frac{ \displaystyle \int_{v_{\min}}^{v_{\max}} v^{1-\alpha} \, dv }{ \displaystyle \int_{v_{\min}}^{v_{\max}} v^{-\alpha} \, dv }$$ Which is expressed as follows (the formula from [@koelmans2020]):
...
The amount of particles in water (per Liter) is then given by dividing the total volume of plastic by the mean volume:
...
```{R Save plots, eval = FALSE, warning=F, message=F}
# In a grid
...
```{R Prepare GC-MS table, warning=F, message=F}
...
```{R Prepare FTIR/LDIR table, warning=F, message=F}
